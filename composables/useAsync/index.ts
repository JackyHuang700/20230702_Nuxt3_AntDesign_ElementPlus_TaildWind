...
type TService<R = any, P extends any[] = any[]> = (...args: P) => Promise<R>
...
export function useAsync<T = any>(pro: TService<T>, options: OptionsType = {}) {
  const {
    manual = false,
    initialData,
  ...
  const run = async () => {
    ...
    try {
      ...
      onError(err)
      state.error = err as unknown as Error
    }
    state.loading = false
  }
